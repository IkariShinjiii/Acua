  ...
  if (signupSuccess) {
    return (
      <div className="text-center py-8 space-y-3 max-w-sm mx-auto">
        ...
        <h3 className="font-serif text-xl text-[#1d1c16]">Check your email</h3>
        <p className="text-sm text-[#57423b]">
          We sent a confirmation link to <strong className="text-[#1d1c16]">{email}</strong>. Confirm it, then log in.
        </p>
        <button
          onClick={() => {
  ...
  return (
    <div className="max-w-sm mx-auto">
      <div className="text-center mb-6">
        ...
        <h2 className="font-serif text-2xl text-[#1d1c16]">
          {title ?? (mode === 'login' ? 'Log In' : 'Create Account')}
        </h2>
        {subtitle && <p className="text-xs text-[#57423b] mt-1.5">{subtitle}</p>}
      </div>

      {allowSignup && (
        <div className="flex gap-2 mb-6 bg-[#f2ede4] rounded-full p-1">
          {['login', 'signup'].map((m) => (
            <button
              key={m}
              type="button"
              ...
              className={`flex-1 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-colors border-none cursor-pointer ${
                mode === m ? 'bg-white text-[#1d1c16] shadow-cloud-sm' : 'text-[#57423b] bg-transparent'
              }`}
            >
